...
TFileStream.prototype = {
  ...
  ReturnFile: function (FileName) {
    if (window.navigator.msSaveBlob) {
      // The following line prompted the user to save the document on IE. It was removed to match Chrome's behavior.
      // return window.navigator.msSaveBlob(new Blob([this.Stream.subarray(0, this.Size)], {type: "application/pdf"}), FileName);
      return new Blob([this.Stream.subarray(0, this.Size)], {type: "application/pdf"});
    } else {
      return new File([new Blob([this.Stream.subarray(0, this.Size)], {type: "application/pdf"})],FileName);
    }
  }
}
...
TPDFKnife.prototype = {
  ...
  SaveToFile: function (FileName, PDFAnalyst, pages) {
    var AStream = new TFileStream();
    var ObjectList = new Array();
    var PDFVersion = '1.4';
    var begin = pages[0];
    ...
    for (var i = 0 ; i < pages.length; i++) {
      if ((i >= 0) && (i <= (PageIDList.length - 1))) {
        ObjectList[ObjectList.length] = PageIDList[pages[i]-1];
      }
    }
    PageIDList = new Array();
    for (var i = 0; i < ObjectList.length; i++) {
    ...
    // ファイルのダウンロード
    return AStream.ReturnFile(FileName);
  }
}
